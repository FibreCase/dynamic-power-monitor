...
- `vite.config.js` — dev-server proxy for `/api`, `/ws` (`ws: true`), `/healthz`, and `/ota` to `127.0.0.1:8000`, so the app can use plain relative paths in both dev and prod and no CORS is ever needed. `build.outDir` is `dist`, matching `config.py`'s `web_dist_dir` default.
